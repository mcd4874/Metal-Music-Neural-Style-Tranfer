...
from utils import mkdir, read_via_scipy, get_spectrogram, get_cepstrogram, get_diff_spectrogram, get_spectral_envelope, \
    plot_figure, ReLU
...
def audio2npys(input_file, config, fname):
    ...
    while st_idx < Len:
        ...
        npy_name = specpath + fname + '_' + str(cnt).zfill(config['num_digit']) + '.npy'
        # print('cnt ={}, max={}'.format(cnt, np.max(list_spec[-1])))

        np.save(npy_name, out_var)
        img_name = imgpath + fname + '_' + str(cnt).zfill(config['num_digit']) + '.png'
